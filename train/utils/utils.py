...
def print_rank_1(msg, rank=1):
    if rank == 1:
        print(msg)

def print_rank_2(msg, rank=2):
    if rank == 2:
        print(msg)

def print_rank_3(msg, rank=3):
    if rank == 3:
        print(msg)

def print_rank_4(msg, rank=4):
    if rank == 4:
        print(msg)

def print_rank_5(msg, rank=5):
    if rank == 5:
        print(msg)

def print_rank_6(msg, rank=6):
    if rank == 6:
        print(msg)

def print_rank_7(msg, rank=7):
    if rank == 7:
        print(msg)
...
def get_tokenizer(model_name_or_path, fast_tokenizer=False):
    if "llama" in model_name_or_path or "Llama" in model_name_or_path:
        ...
        tokenizer.padding_side = 'right'
    assert tokenizer.eos_token is not None
    print(f"{tokenizer.eos_token=}, {tokenizer.pad_token=}, {tokenizer.eos_token_id=}")
    for i in range(7):
        print(f"{tokenizer._convert_id_to_token(i)=}")
    print(f"{tokenizer.convert_tokens_to_ids(['<unk>'])=}, {tokenizer.convert_tokens_to_ids(['<|im_start|>'])=}, {tokenizer.convert_tokens_to_ids(['<|im_end|>'])=}")
    return tokenizer
...
def save_hf_format(model, tokenizer, args, sub_folder=""):
    ...
    # print('another tokenizer')
    # sentence = '<|im_start|> I am a student and I will to go school <|im_end|>'
...
def save_zero_three_model(model_ema, global_rank, save_dir, zero_stage=0):
    ...
    if not zero_stage_3:
        ...
        for k, v in model_to_save.named_parameters():
            # print('key {} with value {}'.format(k, v))
            ...
            # print('output_sd key {} with value {}'.format(k, v_p))
        if global_rank == 0:
            # print('global rank is {}'.format(global_rank))
